...
export default createListViewModel({
  namespace: 'settingMedicationSideEffect',
  config: {
    codetable: {
      message: 'Medication Side Effect updated',
      code: 'ctmedicationsideeffect',
    },
  },
  param: {
    service,
    ...
    effects: {
      *query({ payload }, { put, select, call }) {
        ...
        const settingMedicationSideEffect = yield select(
          st => st.settingMedicationSideEffect,
        )
        const result = yield call(service.queryList, {
          ...payload,
          pageSize:
            payload.pageSize ||
            settingMedicationSideEffect.pagination.pagesize ||
            20,
        })
