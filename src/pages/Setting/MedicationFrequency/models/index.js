...
export default createListViewModel({
  ...
  config: {
    codetable: {
      message: 'Medication Frequency updated',
  ...
  param: {
    ...
    effects: {
      *query({ payload }, { put, select, call }) {
        ...
        const settingMedicationFrequency = yield select(
          st => st.settingMedicationFrequency,
        )
        const result = yield call(service.queryList, {
          ...payload,
          pageSize:
            payload.pageSize ||
            settingMedicationFrequency.pagination.pagesize ||
            20,
        })
