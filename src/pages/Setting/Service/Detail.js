...
@withFormik({
	mapPropsToValues: ({ settingClinicService }) => settingClinicService.entity || settingClinicService.default,
			...
			{ columnName: 'unitPrice', type: 'number', currency: true },
			{
				...
				onRadioChange: (row, e, checked) => {
					...
					if (checked) {
						...
						const serviceSettingItem = _.cloneDeep(values.ctServiceCenter_ServiceNavigation)
			...
			<React.Fragment>
				<SizeContainer size='sm'>
					<div style={{ margin: theme.spacing(1) }}>
						...
						<GridContainer>
							...
							<GridItem md={10}>
								<FastField
									...
									render={(args) => {
										return (
											<DateRangePicker label='Effective Start Date' label2='End Date' {...args} />
										)
									}}
								/>
							</GridItem>
							...
							<GridItem md={6}>
								<FastField
									...
									render={(args) => {
										return (
											<CodeSelect label='Service Category' code='CTServiceCategory' {...args} />
										)
									}}
								/>
							</GridItem>
							<GridItem md={6}>
								<FastField
									...
									render={(args) => {
										return (
											<CodeSelect label='Revenue Category' code='CTRevenueCategory' {...args} />
										)
									}}
								/>
							</GridItem>
						...
						<Divider />
						<GridContainer>
							<GridItem xs={12} md={6} className={classes.detailHeaderContainer}>
								<Checkbox formControlProps={{ className: classes.medisaveCheck }} />
								<FastField
						...
						<GridContainer>
							<GridItem xs={12} md={6} className={classes.detailHeaderContainer}>
								...
								<FastField
									...
									render={(args) => {
										return <Select style={{ paddingLeft: 20 }} prefix='OutPatient Scan' {...args} />
									}}
								/>
							</GridItem>
						</GridContainer>
