...
class Banner extends PureComponent {
  ...
  getAllergyData() {
    ...
    console.log('getAllergyData', entity, allergyData)
    return entity && entity.isActive && (<span style={{ marginTop: 5 }}>{allergyData}</span>)
				...
				<div style={{ display: 'inline-block' }}>
					{data === 'link' ? (
						...
						<div style={{ marginTop: 5 }}>
							{allergyData.length > 25 ? `${allergyData.substring(0, 25).trim()}...` : allergyData}
  ...
  getSchemeList = (schemeDataList) => {
    return schemeDataList.map(s => <Link>
        <span 
          style={{ color: 'black', textDecoration:'underline' }} 
          onClick={(e) => {this.openScheme()}}
        >
          {s.copaymentSchemeName || s.schemeTypeName} {s.validTo ? `(Exp: ${moment(s.validTo).format('DD/MM/YYYY')})` : ''}
        </span>
        {' '}
      </Link>)
  }
	...
	getSchemeDetails = (schemeData) => {
		...
		if (!_.isEmpty(refreshedSchemeData) && refreshedSchemeData.isSuccessful === true) {
			return { ...refreshedSchemeData }
		}

		// Scheme Balance
		const balance =
			schemeData.patientSchemeBalance.length <= 0 ? undefined : schemeData.patientSchemeBalance[0].balance
		...
		const copaymentScheme = copaymentscheme.find((o) => o.id === schemeData.coPaymentSchemeFK)
		return {
			balance,
			...
			acuteBalanceStatusCode:
				!_.isEmpty(refreshedSchemeData) && refreshedSchemeData.isSuccessful === false ? 'ERROR' : undefined,
			chronicBalanceStatusCode:
				!_.isEmpty(refreshedSchemeData) && refreshedSchemeData.isSuccessful === false ? 'ERROR' : chronicStatus,
		...
		if (
			...
			if (refreshData)
				return {
					...
					isSuccessful: refreshData.isSuccessful !== '' ? refreshData.isSuccessful : '',
					schemeTypeName: '',
	...
	displayMedicalProblemData(entity = { patientHistoryDiagnosis: [] }) {
		...
		if (patientHistoryDiagnosis.length > 0) {
			if (patientHistoryDiagnosis.length >= 2) {
				medicalProblemData = `${patientHistoryDiagnosis[0].diagnosisDescription}, ${patientHistoryDiagnosis[1]
		...
		return (
			<div style={{ display: 'inline-block', marginTop: 5 }}>
				...
				{patientHistoryDiagnosis.length > 0 && (
					<Popover
						icon={null}
						content={<HistoryDiagnosis patientHistoryDiagnosis={entity.patientHistoryDiagnosis} />}
						trigger="click"
						placement="bottomLeft"
					>
						<div style={{ display: 'inline-block' }}>
	...
	render() {
      ...
      return {
		...
		const viewPatientProfileAccess = Authorized.check('patientdatabase.patientprofiledetails')

    const	g6PD = codetable.ctg6pd && codetable.ctg6pd.length > 0 && 
    entity.patientAllergyMetaData.length > 0 
    ? codetable.ctg6pd.find((o) => o.id === entity.patientAllergyMetaData[0].g6PDFK) 
    : null
		...
		return (
			...
			<Paper style={style}>
				...
				<GridContainer style={{ minHeight: 100, padding: 5 }}>
            ...
            <CardAvatar testimonial square>
              ...
              style={{
                ...
                paddingRight: 10,
							...
							disabled={
								...
								(viewPatientProfileAccess && viewPatientProfileAccess.rights !== 'enable')
							}
							tabIndex="-1"
						>
						...
						<span className={classes.part}>
							...
							{', '}
							<CodeSelect className={classes.part} text code="ctNationality" value={info.nationalityFK} />
							{', '}
						</span>
						<span className={classes.part}>
                  ...
                  className={classes.part} 
							...
							{', '}
							<DatePicker className={classes.part}  text format={dateFormatLong} value={info.dob} />
						</span>
						<span className={classes.part}>
              <SwitcherTwoTone />
            </span>
              ...
              <span>{info.patientTag.length > 0 ? info.patientTag.map(t => t.tagName).join(', ') : '-'}</span>
          </GridItem>
					<GridItem xs={6} md={3} >
              <span style={{ 
                ...headerStyles,
                color: info.patientMedicalHistory.highRiskCondition ? 'red' : headerStyles.color,
               }}>HRP: </span>
            ...
            <span style={{ 
              ...
              <span>{info.patientHistoryDiagnosis.length > 0 ? info.patientHistoryDiagnosis.map(d => d.diagnosisDescription).join(', ') : '-'}</span>
          </GridItem>
					<GridItem xs={6} md={3} >
            ...
            <span>{this.getAllergyData()}</span>
					...
					<GridItem xs={6} md={3} >
            <Link className={classes.header}>      
              <span style={{textDecoration:'underline'}} onClick={(e) => {this.openPreOrders()}}>
              Pre-Order
              </span>
            </Link>
          </GridItem>
          ...
          <span>{this.getSchemeList(schemeDataList)}</span>
            ...
            Lab Results
				...
				<CommonModal open={this.state.showNotesModal} title="Notes" onClose={this.closeNotes} maxWidth="lg">
					<PatientNurseNotes {...this.props} />
				</CommonModal>
				<CommonModal open={this.state.showPreOrderModal} title="Pre-Orders" onClose={this.closePreOrders} maxWidth="lg">
				</CommonModal>
				<CommonModal open={this.state.showSchemeModal} title="Scheme Details" onClose={this.closeScheme} maxWidth="lg">
				</CommonModal>
			</Paper>
			// </Affix>
			// <GridItem xs={6} md={2}>
