  ...
  showRefreshOrder,
        ...
        const updateTempInvoicePayer = (
          ...
          const newInvoicePayerAmt = newInvoicePayer.reduce((list, n) => {
            const newItems = n.invoicePayerItem
            list.push({
              ...n,
        ...
        const handleSchemeChange = (
          ...
          const schemeConfig = flattenSchemes.find(
            item =>
              item.id === value && payer.schemePayerFK === item.schemePayerFK,
          )
          ...
          const totalClaimed = payerInvoiceItems.reduce(
            (oldTotal, newTotal) => {
        ...
        const toggleCopayerModal = () =>
          setShowCoPaymentModal(!showCoPaymentModal)
        ...
        const syncWithQueried = () => {
          ...
          if (hasAddedPayer) {
            const mapResponseToInvoicePayers = _payer => {
              if (_payer.payerTypeFK !== INVOICE_PAYER_TYPE.COMPANY) {
                ...
                if (claimableSchemes[_claimableSchemesIndex]) {
                  ...
                  ].find(cs => cs.id === _payer.copaymentSchemeFK)
            ...
            if (claimableSchemes.length > 0) {
              ...
              const newInvoicePayers = processAvailableClaims(
                [],
                invoicePayerList,
              )
              setInitialState(newInvoicePayers)
        ...
        const handleCommitChanges = useCallback(
          ({ rows, changed }) => {
            ...
            const mapAndCompareCurrentChangesAmount = item => {
              if (item.id === id) {
                const currentChangesClaimAmount =
                  changed[id].claimAmountBeforeGST
            ...
            const newInvoicePayer = {
              ...
              _hasError:
                rows.reduce(
                  (error, row) =>
                    (row._errors && row._errors.length > 0) || error,
                  false,
                ) || hasError,
            }
        ...
        const onAddPaymentClick = async index => {
          ...
          if (invoicePayer.payerTypeFK === 4)
            selectPayerName = invoicePayer.name
        ...
        const onSubmitVoid = async cancelReason => {
          ...
          if (onVoid.type === 'Payment') {
            let payment = invoicePayer.invoicePayment.find(
              o => o.id === onVoid.id,
            )
              ...
              <CoPayer
                ...
                invoiceItems={updatedInvoiceItems.map(invoiceItem => ({
                  ...
                  payableBalance: roundTo(
                    invoiceItem.totalBeforeGst -
                      (invoiceItem._claimedAmount || 0),
                  ),
                }))}
                invoice={invoice}
              />
