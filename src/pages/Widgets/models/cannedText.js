...
import * as service from '../ClinicalNotes/services/cannedText'
import patientHistoryService from '../../../services/patientHistory'
import { CANNED_TEXT_TYPE_FIELD_NAME } from '../ClinicalNotes/CannedText/utils'
  ...
  param: {
    ...
    reducers: {
      filterDeleted(state, { payload }) {
        ...
        return {
          ...state,
          [fieldName]: (state[fieldName] || []).filter(item => item.id !== id),
        }
      },
