...
class BaseMenu extends PureComponent {
  ...
  getSubMenuOrItem = (item,level )=> {
    ...
    const {location, isMobile, onCollapse,classes,rtlActive,collapsed,color} = this.props
  ...
  getMenuItemPath =(item,level) => {
    ...
    const { target } = item
    const { location, isMobile, onCollapse,rtlActive,classes,collapsed,color } = this.props
    const collapseItemMini =
    `${classes.collapseItemMini 
    } ${ 
    cx({
    ...
    return (
      ...
      <ListItem key={item.path} className={classes.item}>
        ...
        >
          ...
          {typeof item.icon === "string" ? (
            <ListItemIcon className={itemIcon}><Icon>{item.icon}</Icon></ListItemIcon>
                    ) :(<span className={collapseItemMini}>
                      {item.mini || ''}
                        </span>)}
  ...
  render () {
    ...
    return (
      <List className={classes.list}>
        {this.getNavMenuItems(menuData, 0)}
      </List>
    )
  }
}
